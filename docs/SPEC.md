...
### 2.6 予約語
    ...
    OPEN CLOSE LINE CLS LOCATE COLOR SLEEP

予約語は変数名・手続き名に使えない。組込関数名 (§10) は予約語では
ないが、同名の SUB/FUNCTION を定義するとコンパイルエラーになる。
`OPEN` のモード名 `OUTPUT`/`APPEND` は予約語ではなく、OPEN 文の中で
のみ特別な意味を持つ (変数名 `OUTPUT` は合法)。
...
プログラムを即座に終了する。手続きの中からでもよい。終了コードを
省略すると 0。指定した場合は 0〜255 に切り詰められてシェルから
`$?` (Windows では `%ERRORLEVEL%`) として見える。`STOP` は
`END` (コードなし) の別名。
...
### 6.11 画面制御 (CLS / LOCATE / COLOR / SLEEP)

    CLS                    ' 画面を消去してカーソルを左上 (1,1) へ
    LOCATE 行 [, 桁]       ' カーソル移動 (1 起点)。桁省略時は 1
    COLOR 前景 [, 背景]     ' 文字色 (QBasic の 0〜15 パレット)
    SLEEP [秒]             ' 指定秒の待機 (小数可)。省略時はキー入力待ち

- 画面制御は ANSI エスケープシーケンスで実装される。Windows 10 以降の
  コンソールでは起動時に自動で VT モードが有効化される。それより古い
  Windows のコンソールでは動作しない。
- 色番号: 0=黒 1=青 2=緑 3=シアン 4=赤 5=マゼンタ 6=黄(暗) 7=白、
  8〜15 は 0〜7 の高輝度版。負数を渡すとその成分は変更しない。
- 出力がファイルへリダイレクトされている場合、エスケープ列はそのまま
  出力に含まれる (検査はしない)。
- `LOCATE` の行・桁が 1 未満なら実行時エラー "Illegal function call"。
...
- `INPUT` はプロンプト (あれば) に続けて `? ` を表示し、1 行読み込む。
- 複数の変数には、その行のカンマ区切りの値が順に割り当てられる。
  値が足りなければ空とみなす。
- 数値変数への入力が数値として解釈できない場合は 0 になる
  (古典 BASIC の "?Redo from start" 再入力は行わない)。
- `LINE INPUT` は 1 行を**丸ごと** (カンマで分割せず、`? ` も表示せず)
  文字列変数へ読み込む。行末の改行は含まれない。
- どちらも EOF に達した場合は実行時エラー "Out of input (EOF)"。
  標準入力を最後まで処理するプログラムは `EOF(0)` (§10.5) で終端を
  検査してから読むこと:
...
### 9.4 ファイル入出力
    ...
    CLOSE [[#]番号 [, [#]番号 ...]]
    PRINT #番号, [式 [区切り 式 ...]] [区切り]
    INPUT #番号, 変数 [, 変数 ...]
    LINE INPUT #番号, 文字列変数

- ファイル番号は 1〜15。モードは `INPUT` (読み取り。存在しなければ
  実行時エラー "File not found")、`OUTPUT` (新規作成・上書き)、
  `APPEND` (追記)。同じ番号の二重 OPEN は実行時エラー。
- `CLOSE` は指定した番号 (省略時は全部) を閉じる。開いていない番号の
  CLOSE は何もしない。プログラム終了時に開いたままのファイルは自動で
  閉じられる。
- `PRINT #` の書式・区切り (`;` `,`、桁ゾーン、末尾区切りでの改行抑制)
  は画面への PRINT (§9.3) と同一。
- `INPUT #` は項目単位で読む: 項目は引用符付き文字列 (カンマを含める
  場合)、またはカンマ/行末までの生テキスト。**数値変数へ読むときは
  空白も区切りになる** (`10 20 30` が 3 項目で読める古典動作)。
- `LINE INPUT #` は 1 行を丸ごと読む (行末の改行は含まない)。
- 項目・行が残っていない状態で読むと実行時エラー
  "Input past end of file"。事前に `EOF(番号)` で検査すること。
- 読み書きの方向違い (INPUT で開いたファイルへの PRINT # など) は
  実行時エラー。
...
| 関数 | 結果 | 意味 |
|---|---|---|
| `INKEY$` | STRING | 押されているキーが有れば 1 文字、無ければ `""`。待たない |
| `COMMAND$` | STRING | コマンドライン引数全体 (空白 1 個区切りで連結) |
| `COMMAND$(n)` | STRING | n 番目 (1 起点) の引数。範囲外は `""` |
| `EOF(n)` | INTEGER | ファイル n が終端なら真 (-1)。`EOF(0)` は標準入力 |

`INKEY$` と `COMMAND$` も裸の識別子として書ける。

`INKEY$` の注意点:
- 標準入力が端末のとき: エコーせず、押されていなければ即座に `""`。
  矢印キーなどの特殊キーは複数バイトのシーケンスとして 1 バイトずつ
  届く (POSIX では ESC 列、Windows コンソールでは独自の 2 バイト列)。
  可搬なプログラムは印字可能文字だけを判定に使うこと。
- 標準入力がパイプ/リダイレクトのとき: 「次の 1 バイトを読む」動作に
  なる (EOF では `""`)。これによりバッチ実行やテストでも意味を持つ。
...
| Out of input (EOF) | `INPUT`/`LINE INPUT` で標準入力が尽きた |
| Bad file number | ファイル番号が 1〜15 の外 |
| File not open | 開いていないファイル番号への操作 |
| File not opened for input / output | 読み書きの方向違い |
| File not found | `OPEN ... FOR INPUT` の対象が存在しない |
| Cannot open file | `OUTPUT`/`APPEND` で開けない (権限など) |
| File already open | 使用中のファイル番号への OPEN |
| Input past end of file | `INPUT #`/`LINE INPUT #` がファイル終端を越えた |
...
- ランダムアクセスファイル (`OPEN ... FOR RANDOM`、`GET`/`PUT`、
  `FIELD`)、`WRITE #`、`KILL`/`NAME`
- `PRINT USING`、`TAB()`、`SPC()`、`WIDTH`
- 代入文としての `MID$` (`MID$(S$,2,3) = "abc"`)
- `DEF FN`、`DEFINT` などの型宣言文、`OPTION BASE`
- グラフィック (`PSET`/`LINE`/`CIRCLE`)、サウンド (`BEEP`/`PLAY`)
...
statement    = assign | print | input | lineinput | if | while | do
             | for | select | goto | gosub | return | end | dim | const
             | exitstmt | data | read | restore | randomize | swap
             | callstmt | open | close | cls | locate | color | sleep ;
...
print        = "PRINT" [ channel ]
               [ expr { ( ";" | "," ) expr } [ ";" | "," ] ] ;
input        = "INPUT" ( channel lvalue { "," lvalue }
                       | [ string ( ";" | "," ) ] lvalue { "," lvalue } ) ;
lineinput    = "LINE" "INPUT" ( channel lvalue
                              | [ string ( ";" | "," ) ] lvalue ) ;
open         = "OPEN" expr "FOR" ( "INPUT" | "OUTPUT" | "APPEND" )
               "AS" [ "#" ] expr ;
close        = "CLOSE" [ [ "#" ] expr { "," [ "#" ] expr } ] ;
cls          = "CLS" ;
locate       = "LOCATE" expr [ "," expr ] ;
color        = "COLOR" expr [ "," expr ] ;
sleep        = "SLEEP" [ expr ] ;
